...
export async function getRecords(){const s=await tx(STORE);return new Promise<ErrorCode[]>((res,rej)=>{const r=s.getAll();r.onsuccess=()=>res(r.result.map(normalizeRecord));r.onerror=()=>rej(r.error)})}
...
export async function replaceRecords(values:ErrorCode[]){const db=await open();return new Promise<void>((res,rej)=>{const t=db.transaction(STORE,'readwrite');const s=t.objectStore(STORE);s.clear();values.map(normalizeRecord).forEach(v=>s.put(v));t.oncomplete=()=>res();t.onerror=()=>rej(t.error)})}
